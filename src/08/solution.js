...
const processInstruction = (memory, instruction) => {
  const {
    register, operation, ammount, conditionRegister, conditionOperation, conditionAmmount,
  } = instruction;
  ...
  switch (conditionOperation) {
    case '>': conditionPasses = memory[conditionRegister] > conditionAmmount; break;
    case '<': conditionPasses = memory[conditionRegister] < conditionAmmount; break;
    case '>=': conditionPasses = memory[conditionRegister] >= conditionAmmount; break;
    case '<=': conditionPasses = memory[conditionRegister] <= conditionAmmount; break;
    case '==': conditionPasses = memory[conditionRegister] === conditionAmmount; break;
  ...
  if (conditionPasses) {
    switch (operation) {
      case 'inc': memory[register] += ammount; break;
      case 'dec': memory[register] -= ammount; break;
...
const parseInstructions = (input) => {
  ...
  return input
    .split('\n')
    .map((line) => {
      const [, register, operation, ammount, conditionRegister, conditionOperation, conditionAmmount] = line.match(re);
      return {
        register,
        operation,
